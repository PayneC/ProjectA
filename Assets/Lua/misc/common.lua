...
function _M.SetItemIcon(sprite, itemID)
    ...
    if not itemID or itemID == 0 then
        goUtil.SetActiveByComponent(sprite, false)
    else
        goUtil.SetActiveByComponent(sprite, true)
...
function _M.SetCountryIcon(sprite, countryID)
    if not sprite then
        return
    end
    local atlas = 'country'
    local icon = cf_country.GetData(countryID, cf_country.icon)
    if not icon then
        atlas = 'item'
        icon = 'item_0'
    end
...
function _M.SetCountryName(text, countryID)
    if not text then
        return
    end

    local name = cf_country.GetData(countryID, cf_country.name)
    text.text = name or ''
end
