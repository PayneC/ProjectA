...
namespace XLua.CSObjectWrap
{
    ...
    public class EAssetTypeWrap
    {
        ...
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(EAssetType), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(EAssetType), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(EAssetType), L, null, 8, 0, 0);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "NONE", EAssetType.NONE);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "LUA", EAssetType.LUA);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "ENTITY", EAssetType.ENTITY);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "TEXTURE", EAssetType.TEXTURE);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "SCENE", EAssetType.SCENE);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "UI", EAssetType.UI);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "ATLAS", EAssetType.ATLAS);
            
			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(EAssetType), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            if (lua_type == LuaTypes.LUA_TNUMBER)
            {
                translator.PushEAssetType(L, (EAssetType)LuaAPI.xlua_tointeger(L, 1));
            }
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {
			    if (LuaAPI.xlua_is_eq_str(L, 1, "NONE"))
                {
                    translator.PushEAssetType(L, EAssetType.NONE);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "LUA"))
                {
                    translator.PushEAssetType(L, EAssetType.LUA);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "ENTITY"))
                {
                    translator.PushEAssetType(L, EAssetType.ENTITY);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "TEXTURE"))
                {
                    translator.PushEAssetType(L, EAssetType.TEXTURE);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "SCENE"))
                {
                    translator.PushEAssetType(L, EAssetType.SCENE);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "UI"))
                {
                    translator.PushEAssetType(L, EAssetType.UI);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "ATLAS"))
                {
                    translator.PushEAssetType(L, EAssetType.ATLAS);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for EAssetType!");
                }
            }
            else
            {
                return LuaAPI.luaL_error(L, "invalid lua type for EAssetType! Expect number or string, got + " + lua_type);
            }
