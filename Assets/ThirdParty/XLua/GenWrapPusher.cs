...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<EAssetType>(translator.PushEAssetType, translator.Get, translator.UpdateEAssetType);
        ...
        int EAssetType_TypeID = -1;
		int EAssetType_EnumRef = -1;
        
        public void PushEAssetType(RealStatePtr L, EAssetType val)
        {
            if (EAssetType_TypeID == -1)
            {
			    bool is_first;
                EAssetType_TypeID = getTypeId(L, typeof(EAssetType), out is_first);
				
				if (EAssetType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(EAssetType));
				    EAssetType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, EAssetType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, EAssetType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for EAssetType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, EAssetType_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out EAssetType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != EAssetType_TypeID)
				{
				    throw new Exception("invalid userdata for EAssetType");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for EAssetType");
                }
				val = (EAssetType)e;
            ...
            {
                val = (EAssetType)objectCasters.GetCaster(typeof(EAssetType))(L, index, null);
            }
        }
		
        public void UpdateEAssetType(RealStatePtr L, int index, EAssetType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for EAssetType ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(EAssetType[]))
			{
			    EAssetType[] array = obj as EAssetType[];
				translator.PushEAssetType(L, array[index]);
				return true;
			}
            return false;
		}
